...
test('html entity serialization', t => {
  ...
  return reshape({ generator, parser })
    .process(html)
    .then(res => {
      t.truthy(res.output().trim() === '<p foo="&quot;<>">&amp;-&nbsp;-"-&lt;-&gt;-&amp;-&nbsp;-"-&lt;-&gt;</p>\n<p foo="bar"></p>')
    })
})
...
test('return string option', t => {
  const res = generator([{ type: 'text', content: 'hello' }], {
    returnString: true
  })
...
test('self-closing invalid option', t => {
  t.throws(
    () => generator([], { selfClosing: 'snargle' }),
    "'snargle' is an invalid option for 'selfClosing'. You can use 'close', 'tag', or 'slash'"
  )
})
