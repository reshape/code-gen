...
module.exports = function ReshapeCodeGenerator (tree, options = {}) {
  ...
  return options.returnString ? func : (function () { // eslint-disable-line
    'use strict'
    return eval(`${func}.bind(this)`) // eslint-disable-line
  }).call(ctx)
}
...
function walk (tree, options) {
  ...
  return tree.reduce((m, node) => {
    ...
    if (node.type === 'code') {
      ...
      // Note: when returning a string, we can leave the scope open, but when
      // returning a function, it must be bound to `this` to be eval'd correctly
      // within node.js
      let content = options.returnString ? node.content.replace('__runtime', options.runtimeName) : node.content.replace('__runtime', `this.${options.runtimeName}`)
